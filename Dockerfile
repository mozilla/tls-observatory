...
RUN apt-get update -y && apt-get install git libcurl4-nss-dev libnss3 libnss3-dev clang -y && rm -rf /var/lib/apt/lists/* 
RUN git clone https://github.com/mozkeeler/ev-checker.git && cd ev-checker && make && mv ./ev-checker /go/bin/ && cd .. && rm -rf ev-checker
